...
const deleteRowById = async (sheetName, Id, range, index) => {
  try {
    const auth = await google.auth.getClient({
      credentials,
      scopes: ['https://www.googleapis.com/auth/spreadsheets'],
    ...
    const response = await sheets.spreadsheets.values.get({
      spreadsheetId: SPREADSHEET_ID,
      range: `${sheetName}!${range}`, // Adjust the range as needed
    });
    ...
    const indx = await index.toUpperCase().charCodeAt(0) - 'A'.charCodeAt(0);
    const rowsToRemove = values.filter(row => row[indx] === Id);
...
const getDataById = async (sheetName, Id, range, index) => {
  try {
    const auth = await google.auth.getClient({
      credentials,
      scopes: ['https://www.googleapis.com/auth/spreadsheets'],
    ...
    // Filter out the rows with the specified channel ID in the second column
    const indx = await index.toUpperCase().charCodeAt(0) - 'A'.charCodeAt(0);
    const data = values.filter(row => row[indx] === Id);
...
module.exports = { appendToSheet, getSheetData, getDataById, updateUserData, updateSheetData, createSheet, deleteSheet, checkSheetExists, deleteRowById };